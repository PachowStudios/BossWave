/*
 * 
 * OldTVShaderMobileFrag.shader (Mobile Version)
...
Shader "Vortex Game Studios/OLD TV Shader Mobile" {
	Properties {
		...
		_sM ("Statoc Magnetude", Range(-1.0, 1.0)) = 0.5
	...
	SubShader {
		...
		pass { 
			...
			half4 sL(sampler2D c, half2 tC) { 
				...
				iC = tex2D(c, tC);		// We disabled the chromat aberration on mobiles devices,
										// the screen is too small and we got a small performance
										// improvement, but if you want it just uncomment the next line
				//iC = half4(tex2D(tC-tO,c).r*0.8,tex2D(tC,c).g*0.8,tex2D(tC+tO,c).b*0.8,1);
